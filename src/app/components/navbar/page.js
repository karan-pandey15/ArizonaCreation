    ...
    <>
      ...
      <header className="fixed top-0 left-0 w-full bg-white z-50 shadow-md">
        <nav className="flex justify-between items-center w-[95%] mx-auto py-4">
          {/* Logo */}
          <div>
            <Link href="/" className="font-bold text-2xl text-[#111]">
              ArizonaCreation
            </Link>
          </div>
            ...
            <ul className="flex items-center">
              {menuItems.map((item) => (
                <li key={item.name} className={item.name === "Shop" ? "relative" : ""}>
                  <Link
                    href={item.href}
                    className="flex items-center mx-2 lg:mx-5 uppercase text-[14px] lg:text-base tracking-[.15em] font-medium hover:text-[#111] transition-colors"
                  >
                    {item.name}
                    {item.name === "Shop" && (
          ...
          {/* Right Section */}
          <ul className="flex items-center text-[#111] space-x-4">
            <div className="px-3">
              {user ? (
            ...
            {userRole !== "admin" && (
              <li className="cursor-pointer" onClick={() => router.push("/cart")}>
                <div className="relative">
                  <ShoppingCart className="w-6 h-6 text-[#111]" />
                  {distinctCount > 0 && (
            ...
            <ul className="flex-1 space-y-4">
              {menuItems.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="block py-2 text-base font-medium text-gray-700 hover:text-[#111]"
                    onClick={toggleMenu}
                  >
                    {item.name}
                  </Link>
              ...
              {userRole !== "admin" && (
                <li>
                  <Link
                    href="/cart"
                    className="block py-2 flex items-center gap-2 text-base font-medium text-gray-700 hover:text-[#111]"
                    onClick={toggleMenu}
                  >
